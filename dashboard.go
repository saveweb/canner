...
func (s *deliveryStore) dashboardProject(ctx context.Context, project string, cfg projectConfig, now time.Time) (dashboardProject, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*),MIN(CASE WHEN build_error IS NOT NULL THEN next_build_at END) FROM packages WHERE project=? AND state='building'`, project).Scan(&view.BuildingPackages, &nextPackageRetryAt); err != nil {
		return dashboardProject{}, err
	}
