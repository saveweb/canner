...
func (s *deliveryStore) dashboardProject(ctx context.Context, project string, cfg projectConfig, now time.Time) (dashboardProject, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT state,COUNT(*) FROM packages WHERE project=? GROUP BY state`, project)
	if err != nil {
		return dashboardProject{}, err
	}
	for rows.Next() {
		...
		switch state {
		case "building":
			view.BuildingPackages = count
		case "blocked":
			view.BlockedPackages = count
		}
	}
