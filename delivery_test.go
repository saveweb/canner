...
func (s *fakeArtifactSink) deliver(context.Context, deliveryJob, string) (string, error) {
	s.calls++
...
func TestDeliveryWorkerDiscoversAndDeliversAcceptedArtifact(t *testing.T) {
	...
	if len(jobs) != 1 || jobs[0].State != "delivered" || jobs[0].Attempts != 1 || jobs[0].NextAttempt != 0 || jobs[0].RemoteID == nil || *jobs[0].RemoteID != "ia-item" {
...
func acceptTestArtifact(t *testing.T, s *server, body []byte) {
	t.Helper()
	location := createUpload(t, s, testToken, int64(len(body)), blake3Checksum(body))
